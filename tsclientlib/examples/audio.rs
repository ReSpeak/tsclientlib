...
fn main() -> Result<(), failure::Error> {
	...
	runtime.block_on_all(
		futures::lazy(move || {
			let t2a_pipe = ts_to_audio::Pipeline::new(logger2, executor)
				.unwrap();
		...
		.and_then(move |(con, t2a_pipe)| {
			let sink_creator = ConnectionPacketSinkCreator::new(con.clone());
			// TODO Don't unwrap, disconnect if it did not work
			let a2t_pipe = audio_to_ts::Pipeline::new(logger, sink_creator,
				executor2, uri.as_ref().map(|s| s.as_str())).unwrap();
