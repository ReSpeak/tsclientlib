...
impl ReturnCodeHandler {
    ...
    /// received.
    pub(crate) fn get_return_code(&self) -> (usize, oneshot::Receiver<TsError>) {
...
impl<T: 'static> tsproto::handler_data::PacketHandler<T> for
        ...
        S2: Stream<Item=Packet, Error=tsproto::Error> + Send + 'static,
    {
        ...
        let command_stream = command_stream.map(move |p| {
            let is_cmd;
            if let Packet { data: Data::Command(cmd), .. } = &p {
                is_cmd = true;
                    	...
                    	if let Ok(code) = cmd.args["return_code"].parse() {
                    		if let Some(return_sender) =
                    			...
                    			if let Ok(e_code) = cmd.args["id"].parse() {
                    				if let Some(e) = TsError::from_u32(e_code) {
