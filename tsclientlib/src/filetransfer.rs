...
pub(crate) enum FileTransferStatus {
	Start { key: String, port: u16, size: u64, ip: Option<IpAddr> },
	Status { status: TsError },
}
...
impl FileTransferHandler {
	...
	pub(crate) fn get_file_transfer_id(
		...
		let code = FileTransferIdHandle { handler, id: code };
		(code, recv)
	}

	/// Return `true` if the file transfer id was found and the packet was
	/// handled, `false` if not.
	pub(crate) fn handle_start_download(
		&self,
		cmd: &InCommand,
	) -> Result<bool, tsproto::Error>
	{
		let msg = InFileDownload::new(cmd).map_err(|e| {
			format_err!("Cannot parse notifystartdownload ({:?})", e)
		})?;
	...
	/// Return `true` if the file transfer id was found and the packet was
	/// handled, `false` if not.
	pub(crate) fn handle_start_upload(
		&self,
		cmd: &InCommand,
	) -> Result<bool, tsproto::Error>
	{
		let msg = InFileUpload::new(cmd).map_err(|e| {
			format_err!("Cannot parse notifystartdownload ({:?})", e)
		})?;
		let msg = msg.iter().next().unwrap();
	...
	{
		let msg = InFileTransferStatus::new(cmd).map_err(|e| {
			format_err!("Cannot parse notifystatusfiletransfer ({:?})", e)
		})?;
		let msg = msg.iter().next().unwrap();
