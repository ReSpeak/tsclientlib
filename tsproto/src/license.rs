...
impl LicenseKey {
    ...
    pub fn get_pub(&self) -> Result<EdwardsPoint> {
        match *self {
            ...
            LicenseKey::Private(ref k) => Ok(&constants::ED25519_BASEPOINT_TABLE
                * &k.0),
        }
    }
}
...
impl Licenses {
        ...
        while !data.is_empty() {
            ...
            if let Some((start, end)) = bounds {
                ...
                if license.not_valid_before < start
                    ...
                    return Err(format_err!("Invalid license time bounds")
                        .into());
                }
            }
    ...
    pub fn derive_private_key(&self, starting_block: usize,
        first_key: EccKeyPrivEd25519) -> Result<EccKeyPrivEd25519> {
...
impl License {
    ...
    pub fn parse(data: &[u8]) -> Result<(Self, usize)> {
        ...
        let (inner, extra_len) = match data[33] {
            0 => {
                ...
                if license_data > 0x7f {
                    return Err(format_err!("Invalid data in intermediate \
                        license").into());
                }
        ...
        Ok((License {
            ...
            not_valid_before: DateTime::from_utc(NaiveDateTime::from_timestamp(
                i64::from(before_ts) + TIMESTAMP_OFFSET, 0), Utc),
            not_valid_after: DateTime::from_utc(NaiveDateTime::from_timestamp(
                i64::from(after_ts) + TIMESTAMP_OFFSET, 0), Utc),
            hash,
            inner,
        }, all_len))
    }
    ...
    pub fn write(&self, w: &mut Write) -> Result<()> {
        ...
        w.write_u32::<BigEndian>((self.not_valid_before.timestamp()
            - TIMESTAMP_OFFSET) as u32)?;
        w.write_u32::<BigEndian>((self.not_valid_after.timestamp()
            - TIMESTAMP_OFFSET) as u32)?;
    ...
    pub fn derive_private_key(&self, parent_key: &EccKeyPrivEd25519)
