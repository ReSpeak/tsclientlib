	...
	PacketHandler<ServerConnectionData> for DefaultPacketHandler<IPH>
{
		...
		S2: Stream<Item = Packet, Error = Error> + Send + 'static,
	{
		...
		let command_stream = command_stream
			.and_then(move |p| -> Result<Option<Packet>> {
		...
		self.inner
			.new_connection(con_val, command_stream, audio_stream)
	}
}
	...
	fn handle_packet(
		...
		let res = match state.state {
			...
			ServerConnectionState::Init2 { version } => {
				...
				if let Packet {
					data:
						packets::Data::S2CInit(S2CInit::Init3 {
							ref x,
							ref n,
							level,
							ref random2,
			...
			ServerConnectionState::ClientInitIv { ref alpha } => {
				...
				let res =
					(|con_params: &mut Option<ConnectedParams>| -> Result<_> {
						if let Packet {
							data: packets::Data::Command(ref command),
							..
						} = *packet
						{
			...
			ServerConnectionState::Connecting => {
				let mut res = None;
				if let Packet {
					data: packets::Data::Command(ref cmd),
					..
				} = *packet
				{
			...
			ServerConnectionState::Connected => {
				*ignore_packet = false;
				...
				{
					let cmd = cmd.get_commands().remove(0);
					if cmd.command == "notifyclientleftview"
						&& cmd.has_arg("clid")
					{
						// Handle a disconnect
