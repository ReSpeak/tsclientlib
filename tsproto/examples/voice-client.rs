...
struct Args {
	...
	#[structopt(
		...
		short = "u",
		default_value = "",
		help = "The URI of the audio, which should be played. If it is empty, \
		        it will capture from a microphone."
	)]
	uri: String,
	#[structopt(
		long = "volume",
...
impl PacketHandler<client::ServerConnectionData> for MyPacketHandler {
		...
		S2: Stream<Item = Packet, Error = Error> + Send + 'static,
	{
		...
		SimplePacketHandler.new_connection(
			con_val,
			command_stream,
			audio_stream,
		);
...
fn main() {
	...
	runtime.spawn(future::lazy(move || {
		let in_pipe = if !args.no_output {
			...
			let in_pipe =
				create_ts_to_audio_pipeline(executor, logger.clone()).unwrap();
		...
		let in_handler =
			IncommingVoiceHandler::new(logger.clone(), in_pipe).unwrap();

		let ph = MyPacketHandler::new(logger.clone(), in_handler);
		let c =
			create_client(args.local_address, logger.clone(), ph, args.verbose);
		...
		connect(logger.clone(), c.clone(), args.address)
			...
			.and_then(move |con| -> Box<Future<Item = (), Error = ()> + Send> {
				...
				if args.no_input {
					...
					let (pipeline2, audio2) =
						match setup_audio(&args, con.clone(), logger.clone()) {
							Err(error) => {
								error!(logger, "Failed to setup audio"; "error" => ?error);

								// Disconnect
								return Box::new(
									disconnect(con).map(|_| drop(c)).map_err(
										|e| {
											panic!(
												"Failed to connect ({:?})",
												e
...
fn create_audio_to_ts_pipeline(
...
) -> Result<gst::Pipeline, failure::Error>
{
	...
	let volume = gst::ElementFactory::make("volume", "volume")
	...
	let streamvolume =
		volume.dynamic_cast::<gst_audio::StreamVolume>().unwrap();
