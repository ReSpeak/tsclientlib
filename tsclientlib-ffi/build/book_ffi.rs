...
use tsproto_structs::book_to_messages::{
	BookToMessagesDeclarations, Event, RuleKind, RuleOp,
};
use tsproto_structs::*;
...
#[derive(Debug)]
pub struct BookFfi<'a>(
	pub &'a BookDeclarations,
	pub &'a BookToMessagesDeclarations<'a>,
);

impl Default for BookFfi<'static> {
	fn default() -> Self {
		BookFfi(
			&tsproto_structs::book::DATA,
			&tsproto_structs::book_to_messages::DATA,
		)
	}
}
...
fn convert_to_rust(name: &str, type_s: &str) -> String {
	...
	match type_s {
		"str" => {
			format!("unsafe {{ CStr::from_ptr({}).to_str().unwrap() }}", name)
		}
		"Uid" => format!(
			"unsafe {{ UidRef(CStr::from_ptr({}).to_str().unwrap()) }}",
			name
		),
		"ClientId" | "ClientDbId" | "ChannelId" | "ServerGroupId"
		| "ChannelGroupId" | "IconHash" => format!("{}({})", type_s, name),
		"DateTime" => format!(
			"DateTime::from_utc(NaiveDateTime::from_timestamp({}, 0), Utc)",
			name
		),
...
fn get_ffi_arguments_def(r: &RuleKind) -> String {
	match r {
		...
		RuleKind::ArgumentFunction { from, type_s, .. } => format!(
			"{}: {}",
			to_snake_case(from),
