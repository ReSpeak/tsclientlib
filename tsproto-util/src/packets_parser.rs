...
impl Declaration for Packets {
    ...
    fn get_filename() -> &'static str { "Packets.txt" }
...
impl Element {
    ...
    fn get_pre(&self) -> String {
        ...
        if !cond.is_empty() || !cond2.is_empty() {
            ...
            writeln!(&mut res, "\tErr(Error::ParsePacket(String::from(\
                \"Pre condition failed for {}\")))", self.name).unwrap();
            write!(&mut res, "}} else {{\n").unwrap();
        }
    ...
    fn get_post(&self) -> String {
        ...
        if !cond.is_empty() || !cond2.is_empty() {
            ...
            writeln!(&mut res, "\t\tErr(Error::ParsePacket(String::from(\
                \"Post condition failed for {}\")))", self.name).unwrap();
            write!(&mut res, "\t}} else {{\n\
                \t\tOk(res)\n\
                \t}}\n\
                \terror => error,\n\
    ...
    fn wrap_read(&self, inner: &str) -> Result<String> {
        ...
        write!(&mut res, "{{\n\tlet res = ")?;
        if !pre.is_empty() {
            write!(&mut res, "{}{}\t}}", &indent(pre, 1)[1..], indent(inner, 2))?;
        } else {
...
impl Struct {
    ...
    fn write_impl(&self, w: &mut Write, elem: &Element, before: Option<&Field>)
        -> Result<()> {
    ...
    fn write_other_impls(&self, w: &mut Write, before: Option<&Field>)
        -> Result<()> {
    ...
    fn write_raw_read_impl(&self, real_w: &mut Write, elem: &Element,
        before: Option<&Field>) -> Result<()> {
        let mut res = Vec::new();
        {
            let w = &mut res;
    ...
    fn write_raw_write_impl(&self, w: &mut Write) -> Result<()> {
        for fs in &self.fields {
            if !fs.get_type().is_empty() {
                fs.call_write(w, &format!("self.{}", to_snake_case(fs.get_name())))?;
                writeln!(w, ";")?;
            }
        }
        writeln!(w, "Ok(())")?;
    ...
    fn write_raw_debug_impl(&self, w: &mut Write) -> Result<()> {
        for fs in &self.fields {
            if !fs.get_type().is_empty() {
                ...
                fs.call_debug(w, &format!("self.{}", to_snake_case(fs.get_name())))?;
                writeln!(w, ";")?;
                writeln!(w, "write!(f, \", \")?;")?;
            }
        }
...
impl Enum {
    ...
    fn write_raw_read_impl(&self, real_w: &mut Write, elem: &Element,
        ...
        let mut res = Vec::new();
        {
            ...
            writeln!(w, "let reset_cursor = r.clone();\n\
                ...
                let (read, is_struct) = match p.content.element_type {
                    ElementType::Struct(_) => {
                        let read = {
                            ...
                            s.push_str(&format!("{}::{}", elem.name, p.get_name()));
                            s.push_str(&tmp[4..]);
                            s
                        };
            ...
            writeln!(w, "match enum_res {{\n\
                \tSome(res) => Ok(res),\n\
                \tNone => Err(Error::ParsePacket(format!(\"No matching possibility \
                    for enum {} ({{:?}})\", err_buf))),\n\
                }}", elem.name)?;
        }
    ...
    fn write_raw_write_impl(&self, w: &mut Write, elem: &Element) -> Result<()> {
    ...
    fn write_raw_debug_impl(&self, w: &mut Write, elem: &Element) -> Result<()> {
        writeln!(w, "match *self {{")?;
        for p in &self.possibilities {
            let ref_s = if p.is_ref_type() { "ref " } else { "" };
            match p.content.element_type {
...
impl Field {
    ...
    fn read(r: &mut BufRead) -> Result<Option<Self>> {
        if let Some(mut line) = read_line(r)? {
            ...
            if prefix == Some('#') {
                // Enum
                res.content.element_type = ElementType::Enum(Enum::read(&mut Cursor::new(
                    content.as_bytes()))?);
            } else if line.is_empty() {
                // Struct
                res.content.element_type = ElementType::Struct(Struct::read(&mut Cursor::new(
                    content.as_bytes()))?);
            } else {
                // Normal field
                ...
                if first_c == '[' {
                    ...
                    res.content.element_type = ElementType::Array(split_rev(&mut line, pos));
                } else if ['i', 'u'].contains(&first_c) {
                    let pos = line.find(' ').unwrap_or(line.len());
                    res.content.element_type = ElementType::Integer(split_rev(&mut line, pos));
                } else {
                    let pos = line.find(' ').unwrap_or(line.len());
                    res.content.element_type = ElementType::Custom(split_rev(&mut line, pos));
                }
                if !line.is_empty() {
                    res.value = Some(line);
                }
    ...
    fn write_raw_write_impl(&self, w: &mut Write) -> Result<()> {
        match self.content.element_type {
            ElementType::Struct(ref s) =>
                s.write_raw_write_impl(w)?,
            ElementType::Enum(ref s) =>
                s.write_raw_write_impl(w, &self.content.elem)?,
            ElementType::Integer(_) |
            ElementType::Array(_) |
            ElementType::Custom(_) =>
                self.call_write(w, self.get_name())?,
        }
        Ok(())
    }
    ...
    fn write_raw_debug_impl(&self, w: &mut Write) -> Result<()> {
        match self.content.element_type {
            ElementType::Struct(ref s) =>
                s.write_raw_debug_impl(w)?,
            ElementType::Enum(ref s) =>
                s.write_raw_debug_impl(w, &self.content.elem)?,
            ElementType::Integer(_) |
            ElementType::Array(_) |
            ElementType::Custom(_) =>
                self.call_debug(w, self.get_name())?,
        }
        Ok(())
    }
        ...
        match self.content.element_type {
            ...
            ElementType::Array(ref s) => if s.starts_with("Vec<") {
                write!(w, "{}", self.content.elem.wrap_read(&format!(
                    "{{\n\
                    \tlet mut res = Vec::new();\n\
                    \tif let Err(error) = r.read_to_end(&mut res) {{\n\
...
fn read_until_unindent(r: &mut BufRead, s: &mut String)
    -> Result<()> {
